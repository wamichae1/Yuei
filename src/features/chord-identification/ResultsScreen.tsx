...
import { buildChordBreakdown } from "./results";
import { getChordAnswerLabel } from "./theory";
import type {
  ChordAnswerId,
...
export function ResultsScreen({
  ...
  const breakdown = buildChordBreakdown(attempts);
  const message =
    summary.attempted === 0
    ...
    <main className="mx-auto grid w-full max-w-[1200px] flex-1 grid-cols-1 items-start gap-8 px-4 py-10 sm:px-6 lg:grid-cols-12 lg:px-8 lg:py-14">
      <section className="lg:col-span-4 lg:sticky lg:top-8">
        ...
        <p className="mt-5 max-w-md text-base leading-7 text-[var(--slate)]">
          Review the chord breakdown, then repeat this level while the
          contrasts are fresh.
        </p>
      </section>
      ...
      <LiquidCard className="lg:col-span-8 lg:ml-3">
        ...
        <div className="p-5 sm:p-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="technical-label">Chord performance</p>
              <h2 className="mt-1 text-2xl font-semibold tracking-[-0.04em]">
                Weakest answers first
              </h2>
            </div>
          ...
          {attempts.length > 0 && (
            <div className="mt-7">
              ...
              <div className="mt-3 grid grid-cols-4 border-l border-t border-black sm:grid-cols-6">
                {attempts.map((attempt, index) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-semibold">
                      {getChordAnswerLabel(attempt.exercise.correctAnswer)}
                    </span>
                  </div>
                ))}
              </div>
