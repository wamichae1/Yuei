...
interface TrainingScreenProps {
  ...
  timeLabel: string;
  isUnlimited: boolean;
  onAnswer: (answer: IntervalId) => void;
  onPlay: () => void;
  onNext: () => void;
  onEnd: () => void;
  onReset: () => void;
...
export function TrainingScreen({
  ...
  const correct = selectedAnswer === exercise.intervalId;
  const definition = INTERVAL_DEFINITIONS[exercise.intervalId];
      ...
      <div className="grid flex-1 gap-4 lg:grid-cols-12">
        <LiquidCard className="flex min-h-[500px] flex-col lg:col-span-8">
          <div className="border-b-2 border-black p-5 sm:p-6">
            <p className="technical-label text-[var(--slate)]">
              Audio prompt / Interval recognition
            </p>
            <h1 className="mt-2 text-4xl font-semibold tracking-[-0.055em] sm:text-5xl">
              {answered ? "See what you heard" : "What interval do you hear?"}
            </h1>
          </div>
          ...
          <div className="flex flex-1 flex-col p-4 sm:p-6">
            {!answered ? (
              <div className="editorial-grid relative flex min-h-[300px] flex-1 flex-col items-center justify-center overflow-hidden border-2 border-black bg-[var(--paper)] px-5 py-10 text-center">
                ...
                <div
                  aria-hidden="true"
                  className={`grid h-28 w-28 place-items-center rounded-full border-2 border-black bg-white shadow-[0_0_0_12px_var(--green-soft),0_0_0_13px_var(--ink)] transition-transform sm:h-32 sm:w-32 ${
                    isPlaying ? "scale-105" : ""
                  }`}
                >
                  {isPlaying ? (
                ...
                <p className="mt-8 max-w-sm text-lg font-semibold">
                  Listen for the distance and quality between the two
                  pitches.
                </p>
                <button
                  type="button"
                  onClick={onPlay}
                  disabled={isPlaying}
                  className="mt-6 inline-flex min-h-16 min-w-52 items-center justify-center gap-3 rounded-[6px] border-2 border-black bg-[var(--green)] px-7 text-lg font-semibold transition hover:-translate-y-0.5 hover:bg-[var(--yellow)] active:translate-y-0 disabled:cursor-wait disabled:opacity-65"
                >
                  {isPlaying ? (
              ...
              <div className="editorial-grid relative flex min-h-[300px] flex-1 flex-col items-center justify-center overflow-hidden border-2 border-black bg-[var(--paper)] px-2 py-6 sm:px-4">
                ...
                <MusicStaff clef="treble" exercise={exercise} />
                ...
                <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-semibold sm:text-base">
                      {correct
                        ? `Yes — ${definition.label}.`
                        : `The interval was ${definition.label}.`}
                    </p>
                  </div>
        ...
        <aside className="flex flex-col rounded-[10px] border-2 border-black bg-black p-5 text-white sm:p-6 lg:col-span-4">
          ...
          <div className="my-5 grid flex-1 grid-cols-2 content-center gap-2 lg:grid-cols-1 xl:grid-cols-2">
            {options.map((intervalId, index) => {
              const option = INTERVAL_DEFINITIONS[intervalId];
              const isSelected = selectedAnswer === intervalId;
              const isCorrectAnswer =
                answered && exercise.intervalId === intervalId;
              ...
              return (
                ...
                >
                  ...
                  >
                    {ANSWER_SHORTCUTS[index]}
                  </span>
                  <span className="min-w-0">
                    <span className="block font-semibold">
                      {option.shortLabel}
                    </span>
                    <span className="technical-label mt-0.5 block truncate opacity-55">
                      {option.label}
                    </span>
                  </span>
                </button>
          ...
          <p className="technical-label border-t border-white/35 pt-4 leading-5 text-white/45">
            Labeled keys to answer / Spacebar to replay / Enter for next
          </p>
        </aside>
      </div>
    </main>
